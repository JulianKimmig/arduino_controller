...
from multi_purpose_arduino_controller.python_communicator import (
    PythonCommunicator,
...
class ArduinoControllerAPI:
    def __init__(
        ...
        self.data_dir = (
            os.path.join(tempfile.gettempdir(), self.name + "_API")
            if data_dir is None
            else data_dir
        )
        ...
        self.config = (
            JsonDict(file=os.path.join(self.data_dir, "config.json"), createfile=True)
            if config is None
            else config
        )
        ...
        self.set_communicator(
            PythonCommunicator() if python_communicator is None else python_communicator
        )

        self.datalogger = (
            DataLogger(autosave_path=self.data_dir)
    ...
    def set_ports(
        ...
        self.identified_ports = set(
            [] if identified_ports is None else identified_ports
        )
        for port in self.connected_ports.difference(self.sensor_ports):
            self.add_sensor_port(port)
        for port in self.sensor_ports.difference(self.connected_ports):
            self.add_sensor_port(port)
    ...
    def get_board_attribute(self, port, attribute):
        if port not in self.board_data:
            return
